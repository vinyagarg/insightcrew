...
export default function SessionPage({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(`${API_URL}/api/research/${sessionId}/status`, {
          headers: API_HEADERS,
        })
        ...
        if (data.status === 'done') {
          ...
          clearInterval(intervalId)
          setError('This research session could not be found. It may have expired after a server restart.')
        }
      } catch (err) {
        if (isMounted) {
